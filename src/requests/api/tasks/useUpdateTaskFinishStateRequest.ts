...
import type { NoteType, PatchTaskFinishedFormData, Notes, Note, Task } from '@/interfaces'
...
const useUpdateTaskFinishStateRequest = (type?: NoteType) => {
    ...
    async (formData: PatchTaskFinishedFormData): Promise<any> => {
      if (type === 'note') {
        queryClient.setQueryData(
          [NOTES_KEY],
          patchCheckedStatus(queryClient.getQueryData([NOTES_KEY]) as Notes, formData)
        )
      } else if (type === 'archive') {
        queryClient.setQueryData(
          [ARCHIVE_KEY],
          patchCheckedStatus(queryClient.getQueryData([ARCHIVE_KEY]) as Notes, formData)
        )
      } else if (type === 'trash') {
        queryClient.setQueryData(
          [TRASH_KEY],
          patchCheckedStatus(queryClient.getQueryData([TRASH_KEY]) as Notes, formData)
        )
      }
  ...
  const patchCheckedStatus = (notes: Notes, formData: PatchTaskFinishedFormData) => {
    return {
