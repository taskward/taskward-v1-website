...
export default function NoteListCard({
  ...
  const { mutate: archiveNote, isLoading: isArchiveNoteLoading } = useArchiveNoteRequest()
  const { mutate: deleteNote, isLoading: isDeleteNoteLoading } = useDeleteNoteRequest(type)
  const { mutate: unarchiveNote, isLoading: isUnarchiveNoteLoading } = useUnarchiveNoteRequest()
    ...
    <>
      <div
        className={clsx(
          ...
          className,
          focused ? 'drop-shadow-lg dark:drop-shadow-[0_10px_8px_#3a3d41]' : 'drop-shadow-sm',
          isEdit && editable ? 'invisible scale-0 opacity-0' : 'visible scale-100 opacity-100'
        )}
        style={style}
        onClick={() => {
          toggleEdit()
      ...
      >
        {note.name && <div className="block truncate px-4 text-lg font-medium">{note.name}</div>}
        {note.description && (
          <p
