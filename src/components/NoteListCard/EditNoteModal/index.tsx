...
import type { EditNoteFormData, Task, TaskFormData, EditNoteModalProps } from '@/interfaces'
...
export default function EditNoteModal({
  ...
  const { mutateAsync: archiveNoteAsync, isLoading: isArchiveNoteLoading } = useArchiveNoteRequest()
  ...
  const { mutateAsync: unarchiveNoteAsync, isLoading: isUnarchiveNoteLoading } =
    useUnarchiveNoteRequest()
  const { mutateAsync: restoreTrashNoteAsync, isLoading: isRestoreTrashNoteLoading } =
    useRestoreTrashNoteRequest()
  const { mutateAsync: deleteTrashNoteAsync, isLoading: isDeleteTrashNoteLoading } =
    useDeleteTrashNoteRequest()
  ...
  const { handleSubmit, getValues, setValue, reset } = useForm<EditNoteFormData>({
    defaultValues: {
      id: note.id,
      name: note.name,
  ...
  return (
    ...
    >
      ...
      <form
        className={clsx('flex max-h-[380px] flex-col gap-4 overflow-y-auto overflow-x-hidden px-4')}
        ref={scrollRef}
        key={`${isEdit}`}
      >
        <div
            ...
            {tasksData.map((task: TaskFormData) => {
              return (
                <TaskCheckbox
                  ...
                  editable
                  removeTask={() => {
                    setValue('tasks', removeTask(getValues('tasks'), task.id as number))
                    setTasksData(tasksData.filter((taskItem) => taskItem.id !== task.id))
                  }}
                  changeChecked={() => {
                    setValue('tasks', changeChecked(getValues('tasks'), task.id as number))
                  }}
                  changeContent={(content: string | null) => {
                    setValue('tasks', changeContent(getValues('tasks'), task.id as number, content))
                  }}
                  changeLinkUrl={(linkUrl: string | null) => {
                    setValue('tasks', changeLinkUrl(getValues('tasks'), task.id as number, linkUrl))
                  }}
                  copyLinkUrl={() => {
                    copyText(
      ...
      <div className="mt-4 flex justify-between px-4 pb-2 text-xs font-medium dark:text-noteSecondTextDark">
        <div
          className={clsx(
            ...
            isUpdateNoteLoading ? 'visible scale-100 opacity-100' : 'invisible scale-0 opacity-0'
          )}
        >
          <Icon.Sync
            width="16"
      ...
      <div className="border-t-[1px] border-gray-200 px-2 py-1.5 dark:border-gray-600/90">
        {type === 'note' && (
          <NoteListCardPanel
            focused
            note={note}
