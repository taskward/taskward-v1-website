...
export default function Input({
  ...
  return (
    <div className={className}>
      {inputTitle && (
        <span
          className={clsx(
            ...
            required && "after:ml-0.5 after:align-middle after:text-red-500 after:content-['*']"
          )}
        >
          {inputTitle}
        </span>
      ...
      <div
        className={clsx(
          'block select-none text-xs font-medium text-red-500 transition-[opacity,visibility,margin,height] duration-500',
          error ? 'visible mt-1 h-4 opacity-100' : 'invisible h-0 opacity-0'
        )}
      >
        {errorMessage}
      </div>
